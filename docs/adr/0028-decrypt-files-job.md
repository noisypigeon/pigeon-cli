...
- Any bucket-download step (e.g. new `pigeon dataops`/bucket "copy down" tooling) -- this ADR assumes the encrypted files are already present in `--input-dir` by whatever means (manual download, existing S3-compatible tooling, a future ADR).
