...
- Retroactively reconciling an output directory that already has duplicate content from before this ADR's implementation — a one-time "reconcile the existing archive" pass, in the same spirit as ADR-0007's own deferred UID-based reconciliation idea, is a plausible future ADR, not this one.
- Cross-identity deduplication — this stays scoped to one identity's own output tree, matching ADR-0006's flat-per-identity folder structure.
