...
- Decoding mailbox names' modified-UTF-7 encoding into human-readable tags (the `mailbox/...` tag uses the same raw sanitized form sink already produces; prettifying it can follow later without changing this ADR's taxonomy shape).
- Any query/search command over the transformed output.
