...
- **A single shared `ImapSession` today.** Every IMAP call — `examine`, `uid_search`, `uid_fetch`, `list`, `logout` — takes `&mut ImapSession` (`src/email/imap_client.rs`; `src/email/sink.rs:133-134`; `src/email/sync.rs:66`), so one session can only have one command in flight. Concurrent mailboxes need one `ImapSession`/connection per concurrent worker, not a session shared across them.
...
- A per-provider concurrency-cap table in `provider.rs` — accepted non-decision, deferred pending real usage data.
