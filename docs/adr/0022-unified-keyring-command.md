...
**A large structural consequence falls directly out of what's being replaced.** `EmailCommands` (`service/pigeon-cli/src/email/cli.rs`) has exactly two variants today: `Authenticate`, `List`. `DataopsCommands` (`service/pigeon-cli/src/dataops/cli.rs`) has exactly one: `BucketConfig`. Replacing every one of them leaves both enums empty. **`pigeon email` and `pigeon dataops` cease to exist as CLI command groups.** Only `pigeon keyring` and `pigeon job` remain at the top level. The underlying Rust modules that back `job::email_sync` internally — `email::provider`, `email::imap_client`, `email::sink`, `email::transform`, `dataops::client`, `dataops::location`, `dataops::transform`, `dataops::dedup` — are untouched; only each domain's CLI-facing `cli.rs`/`commands.rs`, and `email::identity`'s/`dataops::store`'s `Identity`/`Store` and `BucketConfig`/`Store` metadata-storage types, go away.
...
`service/pigeon-cli/src/cli.rs`'s top-level `Commands` enum drops `Email(EmailArgs)` and `Dataops(DataopsArgs)` entirely and gains `Keyring(KeyringArgs)`, alongside the existing `Job(JobArgs)` (ADR-0021).

### 2. One unified store: `service/pigeon-cli/src/keyring/store.rs`
...
### 3. One keychain service: `service/pigeon-cli/src/keyring/credentials.rs`
...
### 4. `service/pigeon-cli/src/keyring/wizard.rs`: the shared implementation
...
- `pigeon email` and `pigeon dataops` disappear entirely as CLI command groups, since each had nothing left once their sole remaining commands are replaced. `service/pigeon-cli/src/email/{cli,commands}.rs` and `service/pigeon-cli/src/dataops/{cli,commands}.rs` are deleted outright; every other module in both domains (sink, transform, imap_client, provider, client, location, dedup, transform) survives untouched, now serving only as internal plumbing for `job`.
...
- Migrating existing `identities.toml`/`bucket-configs.toml` data or keychain entries into the new unified store — explicitly not built, per the orphaned-data consequence above. ([#23](https://github.com/noisypigeon/pigeon/issues/23))
...
- A flag-driven, non-interactive `modify` or `delete` — both stay interactive-first for both kinds, matching (and, for email, introducing for the first time) `bucket-config edit`'s existing interactive-only precedent. ([#26](https://github.com/noisypigeon/pigeon/issues/26))
