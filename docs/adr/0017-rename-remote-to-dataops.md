...
- Re-exposing `list`/`ls`/`lsd`/`copy`/`list-buckets` as CLI commands anywhere else — not decided here. ([#24](https://github.com/noisypigeon/pigeon/issues/24))
- Any migration/import path for an existing `remotes.toml` or its keychain entries — explicitly not provided. ([#23](https://github.com/noisypigeon/pigeon/issues/23))
