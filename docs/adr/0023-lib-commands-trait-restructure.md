# ADR-0023: restructure into `src/core/` (traits) + `src/commands/` (implementations)
...
This project's module layout has grown organically, one command group at a time: ADR-0008 established one folder per domain (`src/email/`, later `src/dataops/`) with a thin `cli.rs`/`commands.rs` roll-up in each; ADR-0021 added `src/job/` as a sibling, explicitly depending on `email`/`dataops` internals directly rather than through any shared interface; ADR-0022 added `src/keyring/` the same way, unifying `email`'s and `dataops`'s identity/bucket-config storage into one store and one keychain service. Four command-group-shaped folders now exist, each hand-rolling its own version of a few recurring shapes: "a configured secret with an alias" (identity vs. bucket-config), "a job that gathers work and runs it" (only email-sync exists, but the shape is there), "resolve this input from a flag, else prompt, else fall back or error" (five independent implementations across `job::wizard`), and "parse/transform content, dedup it against a durable index" (email message transform + `dataops`'s already-partially-genericized transform/dedup primitives, per ADR-0020).

This ADR restructures the codebase around that recognition: a `src/core/` layer holding trait definitions for these four shared concepts (`keyring`, `job`, `wizard`, `data`) plus whatever infrastructure under each is genuinely kind-agnostic, and a `src/commands/` layer holding the concrete CLI dispatch and per-kind implementations that fulfill those traits. `src/dataops` and `src/email` are removed outright; every file they contain moves into either `src/core/data` (the genuinely generic pieces already identified by ADR-0020) or `src/commands/keyring/{email,bucket}`/`src/commands/job/email_sync` (the concrete, kind-specific pieces).

**Naming note, discovered only once implementation started:** the request's literal `src/lib/keyring` etc. can't actually be declared that way — a module named `lib` cannot be a submodule of `src/lib.rs` itself (`error[E0761]`: the crate-root file and a `mod lib` both resolve to a file named `lib.rs`, an unconditional conflict, not a style choice). Offered three ways out (`src/shared/`, `src/core/`, or a `#[path]` attribute keeping the on-disk name `src/lib/` while the module is spelled differently in code); the user chose **`src/core/`**. Every `src/lib/`/`lib::`-prefixed reference below refers to what is actually `src/core/`/`core::` on disk.
...
```
src/
  lib.rs, main.rs, cli.rs        -- unchanged in spirit; cli.rs's Commands enum now points at src/commands/{keyring,job}
  core/
    ...
    keyring/
      ...
      credentials.rs              -- set_secret/get_secret/delete_secret (moved from src/keyring/credentials.rs, unchanged)
    ...
    wizard.rs                    -- trait WizardInput (see §4); shared read_secret()/confirm() functions (moved from src/keyring/wizard.rs's helpers)
    data.rs                      -- trait Transform, trait Dedup (see §5); ContentIndex, unique_path, sanitize_filename, yaml_quote, collect_files, amend_frontmatter_for_duplicate, rewrite_attachment_reference (moved from src/dataops/{dedup,transform}.rs, unchanged apart from ContentIndex::commit dropping its redundant staging_dir parameter)
  commands/
    ...
    keyring/
      ...
      wizard.rs                   -- add/modify/delete/list handlers (moved from src/keyring/wizard.rs), built on core::wizard's shared pieces
      email/
        mod.rs, identity.rs        -- Identity, sanitize_segment, sanitize_alias
        provider.rs                -- Provider enum (moved from src/email/provider.rs, unchanged)
        imap_client.rs             -- connect_and_login/verify_login (moved from src/email/imap_client.rs, unchanged)
      bucket/
        mod.rs, store.rs           -- BucketConfig struct (moved from src/dataops/store.rs)
        client.rs                  -- S3 operations: bucket_exists, upload_if_changed, list_buckets, list_objects, get_object (moved from src/dataops/client.rs, unchanged)
    job/
      ...
      email_sync/
        ...
        wizard.rs                  -- the five WizardInput impls + dispatch/dispatch_async orchestration (moved from src/job/email_sync.rs's dispatch/dispatch_async + src/job/wizard.rs)
        manifest.rs                -- ManifestEntry/CheckpointEntry/Batch/split_into_batches (moved from src/job/manifest.rs, unchanged)
        sink.rs                    -- fetch_uids, mailbox sanitization, UIDVALIDITY tracking (moved from src/email/sink.rs, unchanged)
        transform.rs               -- EmailTransform: parses .eml, renders markdown+frontmatter; implements core::data::Transform (moved from src/email/transform.rs)
...
### 5. `src/core/data`: the `Transform`/`Dedup` traits
...
### 6. `src/dataops` and `src/email` are removed
...
- Turning the bucket/S3 client operations (`commands::keyring::bucket::client`) into a trait — the request named transform/dedup and email/bucket specifically, not the S3 wire protocol; those operations stay concrete, non-trait functions.
