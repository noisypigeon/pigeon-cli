...
`EmailCommands::ListIdentities` becomes `EmailCommands::List` in `src/email/cli.rs` (clap kebab-cases the variant name automatically, no extra attribute needed).

### `pigeon email sync`: `--local-output <DIR>` replaces `--staging-dir`/`--output-dir`

`--local-output` becomes an optional `PathBuf` in `src/email/cli.rs`. `src/email/commands.rs::sync()` resolves it to `staging_dir = local_output.join("staging")` and `output_dir = local_output.join("result")` once the identity has been resolved (the alias-based default below needs it), applied uniformly across the default flow and both `--debug sink`/`--debug transform` modes.
