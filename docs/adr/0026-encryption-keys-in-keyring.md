...
- Key rotation propagating to already-uploaded ciphertext -- rotating `modify encryption-key`'s secret only affects *future* uploads; old objects stay encrypted under whatever key was active when they were uploaded, so decrypting them later still requires that old key under some alias. ([#31](https://github.com/noisypigeon/pigeon/issues/31))
