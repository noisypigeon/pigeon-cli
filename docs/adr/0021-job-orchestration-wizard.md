...
- A generic job-type plugin/registry CLI (e.g. `pigeon job list-types`) — only `email-sync` is implemented; the module boundary allows more, but no second job type is built now. ([#28](https://github.com/noisypigeon/pigeon/issues/28))
- Historical-throughput-based estimate refinement — the estimate stays a static heuristic in this ADR. ([#29](https://github.com/noisypigeon/pigeon/issues/29))
