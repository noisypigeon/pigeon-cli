...
- Sanitizing other untrusted MIME-derived strings not implicated in this bug (sender display name, subject) — those already flow through the existing `sanitize_segment`/`yaml_quote` paths and aren't affected by this class of failure. ([#19](https://github.com/noisypigeon/pigeon/issues/19))
