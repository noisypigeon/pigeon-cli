...
- `transform` itself.
- Deleting, expiring, or otherwise mutating local files sink has already written (beyond the documented stale-`UIDVALIDITY` re-fetch case). ([#7](https://github.com/noisypigeon/pigeon/issues/7))
