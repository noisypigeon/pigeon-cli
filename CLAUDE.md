...
- `docs/adr/0035-pretrust-keychain-access.md` — root-causes `job run email-sync` prompting for the macOS keychain password 5-10 times per run (one distinct `keyring::Entry` per identity/bucket-config/encryption-key alias, each needing its own OS-level access grant). Decision: on macOS, `set_secret`/`get_secret`/`delete_secret` shell out to `security add-generic-password -T <current-binary-path>`/`find-generic-password`/`delete-generic-password` instead of the `keyring` crate, so writing a secret pre-authorizes the running binary and future reads never prompt; non-macOS unchanged.
